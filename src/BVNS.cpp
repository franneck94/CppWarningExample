      ...
      m_local_solution(m_locationNumber, 1)
{
    ...
    initialSolution(m_incumbent_solution,
                    m_bi,
                    m_fi,
                    m_dj,
                    m_cij,
    ...
    for (size_t i = 0; i != tMax && !stop; ++i)
    {
        ...
        while (m_k < m_kMax && !stop)
        {
            ...
            updateNeighborhoods(m_neighborhood_k,
                                m_perturbed_solution,
                                m_bi,
                                m_sum_dj,
                                m_shaking_mode,
                                m_k);
            ...
            m_local_solution = localSearch(m_perturbed_solution,
                                           ...
                                           m_local_search_mode);
            // Move or not
            neighborhoodChange(m_incumbent_solution,
                               m_local_solution,
                               m_k,
                               m_fx,
                               m_best_fx,
            ...
            chrono::steady_clock::time_point bvns_step =
                chrono::high_resolution_clock::now();
            double time_end =
                (double)chrono::duration_cast<chrono::microseconds>(bvns_step -
                                                                    timeStart)
                    .count();
    ...
    updateXij(m_cij,
              m_dj,
              m_bi,
              m_incumbent_solution,
              m_update_xij_mode,
              m_best_fx);

    // Save Solution and Timestamps to File
    printFinalSolution(m_incumbent_solution,
                       m_best_fx,
                       m_kMax,
                       m_tMax,
                       m_shaking_mode,
...
vector<bool> BVNS::localSearch(const vector<bool> &perturbed_solution,
                               ...
                               const unsigned int &local_search_mode)
{
    if (local_search_mode == 0)
        return localSearchFirstImprovment(perturbed_solution,
                                          neighborhood_k,
                                          k,
                                          fx);
    else
        return localSearchBestImprovment(perturbed_solution,
                                         neighborhood_k,
                                         k,
                                         fx);
}
