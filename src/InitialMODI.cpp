...
/**********************
	...
	**********************/
void _initVogel(vector<double> &bi,
                vector<double> dj,
                TsBasic *basicsEnd,
                TsBasic **facBasics,
                TsBasic **cusBasics,
...
{
    int i, j;
    TsVogPen *srcPens = nullptr;
    TsVogPen *snkPens = nullptr;
    TsVogPen *pitra = nullptr, *pitrb = nullptr; //iterators
    TsVogPen *maxPen = nullptr;
    ...
    for (pitra = srcPenHead.next, i = 0; pitra != NULL;
         pitra = pitra->next, i++)
    {
        for (pitrb = snkPenHead.next, j = 0; pitrb != NULL;
             pitrb = pitrb->next, j++)
        {
    ...
    {
        maxIsSrc = true;
        for (maxPen = pitra = srcPenHead.next; pitra != NULL;
             pitra = pitra->next)
            if ((pitra->twoCost - pitra->oneCost) >
                (maxPen->twoCost - maxPen->oneCost))
                maxPen = pitra;
             ...
             fabs(dj[i] - bi[j]) < _tsMaxW * TSEPSILON))
        {
            ...
            for (pitra = snkPenHead.next; pitra != NULL; pitra = pitra->next)
            {
                if (pitra->one == i || pitra->two == i)
                {
                    ...
                    for (pitrb = srcPenHead.next; pitrb != NULL;
                         pitrb = pitrb->next)
        ...
        else
        {
            ...
            for (pitra = srcPenHead.next; pitra != NULL; pitra = pitra->next)
            {
                if (pitra->one == j || pitra->two == j)
                {
                    pitra->oneCost = TSINFINITY;
                    pitra->twoCost = TSINFINITY;
                    for (pitrb = snkPenHead.next; pitrb != NULL;
                         pitrb = pitrb->next)
        ...
        if (cusBasics[j] != NULL)
            cusBasics[j]->prevCus = basicsEnd;
        basicsEnd->nextFac = facBasics[i];
        if (facBasics[i] != NULL)
            facBasics[i]->prevFac = basicsEnd;
    ...
    while (i < m && j < n)
    {
        ...
        if (bi[i] >= dj[j] && dj[j] != 0.0)
        {
            ...
            if (bi[i] == 0.0 && dj[j] == 0.0 && i < m - 1 && j < n - 1)
            {
                ...
                if (cusBasics[j + 1] != NULL)
                    cusBasics[j + 1]->prevCus = basicsEnd;
                basicsEnd->nextFac = facBasics[i];
                if (facBasics[i] != NULL)
                    facBasics[i]->prevFac = basicsEnd;
...
void _initLCM(double *bi,
              ...
              int m,
              int n,
              Matrix<unsigned int> &cij_column_sorted,
              int *facilities)
{
    unsigned int i = 0, j = 0;
